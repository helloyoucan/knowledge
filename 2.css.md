...
- **flex-nasis**:auto |<number> ;（在分配多余空间之前，项目占据的主轴空间，浏览器根据这个属性计算主轴多余空间）
...
- 指定项目的四个边框，分别定位在哪根网格线。（指定项目的位置）
  ...
  **grid-column-start**:<number>;（左边框所在的垂直网格线）
  **grid-column-end**:<number>;（右边框所在的垂直网格线）
  **grid-row-start**:<number>;（上边框所在的水平网格线）
  **grid-row-end**:<number>;（下边框所在的水平网格线）
